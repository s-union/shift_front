            ...
            <form onSubmit={handleSubmit} className='max-w-md w-full mx-auto bg-white p-8 mb-8 rounded-lg shadow-lg border-4'>
                ...
                <div className='flex items-center justify-between'>
                    <input
                        type='submit'
                        value='送信'
                        className={`font-bold py-3 px-6 rounded-lg focus:outline-none focus:shadow-outline transition duration-300 ease-in-out transform hover:scale-105 ${
                            isDisabled ? 'bg-gray-600 cursor-not-allowed' : 'bg-cyan-500 hover:bg-cyan-600 text-white'
                        }`}
                        disabled={isDisabled}
                    />
                </div>
